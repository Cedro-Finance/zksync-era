...
impl FromEnvChain for EcosystemContracts {
    fn from_env_chain(chain: Chain) -> anyhow::Result<Self> {
        match chain {
            ...
            Chain::BNB => Ok(Self {
                bridgehub_proxy_addr: std::env::var("CONTRACTS_BNB_BRIDGEHUB_PROXY_ADDR")?
                    .parse()?,
                state_transition_proxy_addr: std::env::var(
                    "CONTRACTS_BNB_STATE_TRANSITION_PROXY_ADDR",
                )?
                .parse()?,
                transparent_proxy_admin_addr: std::env::var(
...
impl FromEnvChain for ContractsConfig {
    fn from_env_chain(chain: Chain) -> anyhow::Result<Self> {
        match chain {
            Chain::ETH => {
                let mut contracts: ContractsConfig = envy_load("contracts", "CONTRACTS_ETH_")?;
                // Note: we are renaming the bridge, the address remains the same
                // These two config variables should always have the same value.
                // TODO(EVM-578): double check and potentially forbid both of them being `None`.
                contracts.l2_erc20_bridge_addr = contracts
                ...
                if let (Some(legacy_addr), Some(shared_addr)) = (
                    ...
                    if legacy_addr != shared_addr {
                        panic!(
                            "L2 erc20 bridge address and L2 shared bridge address are different."
                        );
                    }
                }
                contracts.ecosystem_contracts = EcosystemContracts::from_env().ok();
            ...
            Chain::BNB => {
                let mut contracts: ContractsConfig = envy_load("contracts", "CONTRACTS_BNB_")?;
                // Note: we are renaming the bridge, the address remains the same
                // These two config variables should always have the same value.
                // TODO(EVM-578): double check and potentially forbid both of them being `None`.
                contracts.l2_erc20_bridge_addr = contracts
...
CONTRACTS_ETH_BASE_TOKEN_ADDR="0x0000000000000000000000000000000000000001"
        ...
        let actual = ContractsConfig::from_env_chain(Chain::ETH).unwrap();
        assert_eq!(actual, expected_config());
    }
}
